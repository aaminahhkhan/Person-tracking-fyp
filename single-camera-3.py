...
# Check for CUDA availability and install CUDA-enabled PyTorch if needed
if not torch.cuda.is_available():
    logging.warning("CUDA not available. Attempting to install CUDA-enabled PyTorch...")
    try:
        ...
        logging.info("CUDA-enabled PyTorch installed. Please restart the script.")
        sys.exit(0)
    except Exception as e:
        logging.error(f"CUDA install failed: {e}. Falling back to CPU.")
...
def main():
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    logging.info(f"Running on {'CUDA' if device == 'cuda' else 'CPU'} device")
    
    reid_system = PersonReID(device=device)

    cap = cv2.VideoCapture('./videos/test-video-2.mp4')
    frame_id = 0
